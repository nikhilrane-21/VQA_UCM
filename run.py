...
def train():
    ...
    for epoch in range(args.num_epochs):
        ...
        for i_batch, sample_batched in enumerate(tqdm(ucm_vqa_train_dataloader)):
...
def eval():
    ...
    with torch.no_grad():
        ...
        with open(os.path.join(args.model_dir, 'model-epoch-{:02d}-evaluation.txt'.format(args.num_epochs)),
                  "w") as file:
